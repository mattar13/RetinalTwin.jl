...
# Arguments
- `params`: NamedTuple from `load_all_params()`
...
function retinal_column_initial_conditions(params)
    ...
    ic_size = 0
    u0_photoreceptor = photoreceptor_state(params.PHOTO)
    ...
    u0_on_bipolar = on_bipolar_state(params.ONBC)
    ...
    u0_off_bipolar = off_bipolar_state(params.OFFBC)
...
function (RCM::RetinalColumnModel)(du, u, p, t)
    ...
    for cell in values(RCM.cells)
        if cell.cell_type == :PC
            ...
            photoreceptor_model!(duc, uc, (params.PHOTO, pc_stim), t)
        elseif cell.cell_type == :HC
            ...
            I_gap = params.HC.g_gap *
                sum(w * (get_out(u, RCM.cells[pre], key) - V_hc) for (pre, key, w) in inputs if key == :V)
            horizontal_model!(duc, uc, (params.HC, I_gap, glu_in, w_glu_in), t)
        end
        if cell.cell_type == :ONBC
            ...
            I_gap = params.A2.g_gap *
                sum(w * (get_out(u, RCM.cells[pre], key) - V_onbc) for (pre, key, w) in inputs if key == :V)
            on_bipolar_model!(duc, uc, (params.ONBC, glu_in, w_glu_in), t)
            duc[ONBC_IC_MAP.V] += I_gap / params.ONBC.C_m
        elseif cell.cell_type == :OFFBC
            ...
            glu_in = [get_out(u, RCM.cells[pre], key) for (pre, key, _) in inputs if key == :Glu]
            w_glu_in = [w for (_, key, w) in inputs if key == :Glu]
            off_bipolar_model!(duc, uc, (params.OFFBC, glu_in, w_glu_in), t)
        elseif cell.cell_type == :A2
            ...
            a2_model!(duc, uc, (params.A2, glu_in, w_glu_in), t)
            duc[A2_IC_MAP.V] += I_gap / params.A2.C_m
        elseif cell.cell_type == :GC
            ...
            ganglion_model!(duc, uc, (params.GC, glu_in, w_glu_in, gly_in, w_gly_in), t)
        elseif cell.cell_type == :MG
            ...
            for src in values(RCM.cells)
                usc = uview(u, src)
                if src.cell_type == :PC
                    K_photo_efflux += photoreceptor_K_efflux(usc, params.PHOTO)
                elseif src.cell_type == :ONBC
                    K_onbc_efflux += on_bipolar_K_efflux(usc, params.ONBC)
                    glu_gc_exc += get_out(u, src, :Glu)
                elseif src.cell_type == :OFFBC
                    K_offbc_efflux += off_bipolar_K_efflux(usc, params.OFFBC)
                    glu_gc_exc += get_out(u, src, :Glu)
                elseif src.cell_type == :A2
                    K_a2_efflux += a2_amacrine_K_efflux(usc, params.A2)
                elseif src.cell_type == :GC
                    K_gc_efflux += ganglion_K_efflux(usc, params.GC)
                end
            end
            ...
            muller_model!(duc, uc, (params.MULLER, I_K_src_end, I_K_src_stalk, glu_gc_exc), t)
        end
    end
